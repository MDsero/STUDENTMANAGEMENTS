...
create table public.classes (
  ...
  title text not null, instructor text, room text, starts_at timestamptz not null, ends_at timestamptz not null,
...
alter table public.profiles enable row level security; alter table public.students enable row level security; alter table public.programs enable row level security;
...
create or replace function public.dashboard_metrics() returns jsonb language sql stable security invoker set search_path=public as $$
 select jsonb_build_object('students',(select count(*) from students where status='active'),'programs',(select count(*) from programs where is_active),'todayClasses',(select count(*) from classes where starts_at::date=current_date),'attendanceToday',(select count(*) from attendance where attendance_date=current_date and status='present')); $$;
